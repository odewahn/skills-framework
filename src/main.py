...
# 'docs' command: generate {README_GENERATED_NAME} by injecting artifact templates into {README_TEMPLATE_NAME}.
...
def generate_docs():
    f"""Generate {README_GENERATED_NAME} by injecting the artifact templates into {README_TEMPLATE_NAME}."""
    ...
    context = {}
    for jinja_path in glob.glob(os.path.join(ARTIFACT_PATH, f"*{JINJA_EXTENSION}")):
...
def create_artifact(slug, artifact, display_topic, metadata=None):
    ...
    generators_dir = os.path.join(base_dir, GENERATORS_DIR_NAME)
    os.makedirs(generators_dir, exist_ok=True)

    artifact_template = load_file(
        os.path.join(ARTIFACT_PATH, artifact + JINJA_EXTENSION)
    )
    ...
    prompt_path = os.path.join(generators_dir, artifact + MARKDOWN_EXTENSION)
    write_file(prompt_path, rendered_output)

    result_path = os.path.join(base_dir, artifact + MARKDOWN_EXTENSION)
    write_file(
        result_path,
        f"{NO_CHATGPT_MSG}\n\n"
        f"Paste {artifact}{MARKDOWN_EXTENSION} from chat.openai.com output and EDIT OUT ANY CRUFT",
    )
...
def main():
    ...
    if args.command == "render":
        ...
        # Skill Framework is the first artifact and does not require prior ChatGPT output
        if args.artifact == SKILL_FRAMEWORK:
